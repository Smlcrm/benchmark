...
if __name__ == "__main__":
  ...
  test_cases.append((data_loader_to_be_tested.load_single_chunk(1).train.targets["y"], pd.Series([1, 2, 3000, 7001, 1, 2, 3000, 7001, 1, 2, 3000, 7001, 1, 2, 3000, 7001])))
  test_cases.append((data_loader_to_be_tested.load_single_chunk(3).train.targets["y"],pd.Series([[8, 15], [7, 4], [8, 15], [7, 4], [8, 15], [7, 4], [8, 15], [7, 4],[8, 15]])))

  two_chunks = data_loader_to_be_tested.load_several_chunks(2)
  chunk_one, chunk_two = two_chunks

  test_cases.append((chunk_one.validation.targets["y"], pd.Series([1,2],index=[16,17])))
  test_cases.append((chunk_two.test.targets["y"], pd.Series([9,4,8,11,15],index=[37,38,39,40,41])))
