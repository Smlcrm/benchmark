...
class ExponentialSmoothingModel(BaseModel):
    def __init__(self, config: Dict[str, Any] = None, config_file: str = None):
        ...
        if "model" in self.config and "exponential_smoothing" in self.config["model"]:
            ...
            raise ValueError(
                "Model configuration for 'exponential_smoothing' must be specified in the config file under the 'model' section. Model execution aborted."
            )
        ...
        self.seasonal_periods = _cast_param(
            "seasonal_periods", model_config.get("seasonal_periods", None)
        )
        self.damped_trend = _cast_param(
            "damped_trend", model_config.get("damped_trend", False)
        )
        self.forecast_horizon = _cast_param(
    ...
    def train(
        ...
        **kwargs,
        ...
        if isinstance(seasonal, str) and seasonal.lower() == "none":
            seasonal = None
        seasonal_periods = (
            int(self.seasonal_periods) if self.seasonal_periods is not None else None
        )
    ...
    def predict(
        ...
        **kwargs,
        ...
        print(
            f"[ExponentialSmoothing predict] y_context type: {type(y_context)}, shape: {getattr(y_context, 'shape', 'N/A')}"
        )
        print(
            f"[ExponentialSmoothing predict] y_target type: {type(y_target)}, shape: {getattr(y_target, 'shape', 'N/A')}"
        )
        ...
        if y_target is None:
            raise ValueError(
                "y_target must be provided to determine prediction length."
            )
        ...
        try:
            ...
            print(
                f"[ExponentialSmoothing predict] forecast type: {type(forecast)}, shape: {getattr(forecast, 'shape', 'N/A')}"
            )
            print(
                f"[ExponentialSmoothing predict] forecast first 5 values: {forecast[:5] if hasattr(forecast, '__getitem__') else forecast}"
            )
            ...
            print(
                f"[BEFORE ExponentialSmoothing predict] result shape: {forecast_array.shape}"
            )
            if len(forecast_array.shape) == 1:
                result = np.expand_dims(forecast_array, axis=1)
