"""
Main script for orchestrating the end-to-end benchmarking pipeline for Chronos.
"""
import os
import time
import yaml
...
from benchmarking_pipeline.models.chronos.chronos_model import ChronosForecaster  # assumes the class lives here
...
class BenchmarkRunner:
    ...
    def run(self):
        ...
        if len(all_dataset_chunks) > 0:
            try:
                feat_shape = all_dataset_chunks[0].train.features.shape
            except Exception:
                feat_shape = None
            print(f"[DEBUG] First chunk train shape: {feat_shape}")
...
def run_chronos(all_dataset_chunks, writer=None, config=None, config_path=None):
    dataset_name = config['dataset']['name']
    chronos_params = config['model']['parameters']['Chronos']
    prediction_length = chronos_params.get('forecast_horizon', 24)

    model_size = chronos_params.get('model_size', 'small')
    context_length = chronos_params.get('context_length', 64)
    num_samples = chronos_params.get('num_samples', 10)
    ...
    chronos_model = ChronosForecaster(
    ...
    if hasattr(chunk.train, "features"):
        ...
        df = pd.DataFrame(chunk.train.labels)

    # Run prediction
    results = chronos_model.predict(df, prediction_length=prediction_length)
    print(f"Chronos predictions for {dataset_name}:")
    for series_name, forecast in results.items():
        print(f"  {series_name}: {forecast[:5]} ...")

    # Log to TensorBoard
    if writer is not None:
        for series_name, forecast in results.items():
            scalar = float(np.asarray(forecast).flatten()[0])
            writer.add_scalar(f'Chronos/{series_name}_first_pred', scalar, 12)
        if hasattr(chunk.test, "features"):
            ...
            # optional comparison plotting

    os.makedirs('results', exist_ok=True)
    pd.DataFrame(results).to_csv(f'results/Chronos_{dataset_name}_{time.strftime("%Y%m%d-%H%M%S")}.csv', index=False)
    print("CHRONOS WORKS!")
