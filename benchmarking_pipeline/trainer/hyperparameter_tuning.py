...
class HyperparameterTuner:
  def __init__(self, model_class: BaseModel, hyperparameter_ranges: Dict[str, List], use_exog: bool, is_statsmodel: bool = True):
    """
    ...
    self.is_statsmodel = is_statsmodel
  ...
  def hyperparameter_grid_search_single_time_series(self, time_series_dataset):
    ...
    for hyperparameter_setting in itertools.product(*self.hyperparameter_ranges.values()):
      ...
      trained_model = self.model_class.train(y_context=target, x_context=None)

      validation_series = time_series_dataset.validation.features[self.model_class.target_col]

      model_predictions = trained_model.predict(y_context=validation_series) if self.use_exog else trained_model.predict(None)
      current_train_loss = trained_model.compute_loss(y_true=time_series_dataset.validation.features[self.model_class.target_col], y_pred=model_predictions)
  ...
  def hyperparameter_grid_search_several_time_series(self, list_of_time_series_datasets):
    ...
    for time_series_dataset in list_of_time_series_datasets:
      validation_score, hyperparameters = self.hyperparameter_grid_search_single_time_series(time_series_dataset=time_series_dataset)
  ...
  def final_evaluation(self, best_hyperparamters: Dict[str, int], list_of_time_series_datasets):
    ...
    for time_series_dataset in list_of_time_series_datasets:
      ...
      predictions = trained_model.predict(None)
      train_loss_dict = trained_model.compute_loss(y_true=time_series_dataset.test.features[self.model_class.target_col], y_pred=predictions)
