...
class FoundationModelTuner:
    def __init__(
        self, model_class, hyperparameter_ranges: Dict[str, List], use_exog: str
    ):
        """
    ...
    def _get_target_data(self, dataset_split):
        ...
        if dataset_split.targets is not None:
            ...
            if hasattr(dataset_split.targets, "values"):
                ...
                raise ValueError(
                    f"Unexpected targets format: {type(dataset_split.targets)}"
                )
        else:
            raise ValueError("No targets available in dataset split")

    def hyperparameter_grid_search(self, list_of_time_series_datasets):
        """
            ...
            for hyperparameter_setting in itertools.product(
                *self.hyperparameter_ranges.values()
                ...
                if time_series_dataset.train.features is not None:
                    print(
                        f"DEBUG: train.features columns: {time_series_dataset.train.features.columns}"
                    )
                    print(
                        f"DEBUG: train.features head:\n{time_series_dataset.train.features.head()}"
                    )
                else:
                    print(f"DEBUG: train.features is None, using train.targets")
                    print(
                        f"DEBUG: train.targets shape: {time_series_dataset.train.targets.shape if time_series_dataset.train.targets is not None else 'None'}"
                    )

                # Get target data from raw arrays (no column names needed)
                target = self._get_target_data(time_series_dataset.train)
                validation_series = self._get_target_data(
                    time_series_dataset.validation
                )
                ...
                for time_series_dataset_from_all in list_of_time_series_datasets:
                    ...
                    validation_series = self._get_target_data(
                        time_series_dataset_from_all.validation
                    )
                    ...
                    validationum_targets = self._get_target_data(
                        time_series_dataset_from_all.validation
                    )
                    train_targets = self._get_target_data(
                        time_series_dataset_from_all.train
                    )
                    train_loss = trained_model.compute_loss(
                        validationum_targets, model_predictions, y_train=train_targets
                    )
    ...
    def final_evaluation(
        ...
        for time_series_dataset in list_of_time_series_datasets:
            ...
            # For foundation models, we need to call train() to initialize the model
            trained_model.train(
                y_context=target, y_target=validation_data, y_start_date=y_start_date
            )
            ...
            train_loss_dict = trained_model.compute_loss(
                test_targets, predictions, y_train=train_data
            )
