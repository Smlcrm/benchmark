        ...
        for task_type_dir in model_type_dir.iterdir():
            ...
            for model_dir in task_type_dir.iterdir():
                ...
                if metrics_file.exists():
                    metrics_files.append((metrics_file, model_type, task_type, model_name))
...
def unify_metrics(metrics_dir, output_file):
    ...
    for file_path, model_type, task_type, model_name in metrics_files:
        ...
        metrics = extract_metrics_from_file(file_path, model_type, task_type, model_name)
        if metrics:
...
def main():
    parser = argparse.ArgumentParser(description='Unify metrics from nested folder structure')
    parser.add_argument('--metrics-dir', default='benchmarking_pipeline/metrics', 
                       help='Path to the metrics directory (default: benchmarking_pipeline/metrics)')
    parser.add_argument('--output', default='unified_metrics.csv',
