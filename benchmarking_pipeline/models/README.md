...
| **TabPFN** | A pre-trained Transformer model that performs in-context learning on tabular datasets. It can make predictions on univariate and multivariate data. | `tabpfn_ts, tabpfn` |
...
| **TimesFM** | A pre-trained, decoder-only Transformer foundation model developed by Google Research for time-series forecasting. | `transformers` |
